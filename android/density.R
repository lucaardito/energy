...
for(i in 1:length(data.sources)){
  ...
  discr.power = dens$x[gap] + (dens$x[dens$peaks[3]]-dens$x[dens$peaks[2]])/4
  ...
  for(j in 1:length(dens$peaks)){
    rp <- rp + annotate("segment", x=0,xend=length(data$V1),y=dens$x[dens$peaks[j]],yend=dens$x[dens$peaks[j]], color="green",linewidth=2)
  }
  rp <- rp + annotate("segment", x=0,xend=length(data$V1),y=discr.power,yend=discr.power, color="red",linewidth=2)
